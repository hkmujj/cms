            ...
            {loop $list $vo}
             <li>
                 <div class="media media-x">
                     {if $vo['picture']}<a class="float-left" title="{$vo['title']}" href="{$vo['url']}" target="_blank"><img src="{$vo['picturepath']}" class="radius" alt="{$vo['title']}"></a>{/if}
                     <div class="media-body">
                     <strong class="float-left"><?php echo str_replace($keywords,"<font style='color:red'>$keywords</font>",$vo['title']); ?></strong><span class="text-gray float-right">{date($vo['addtime'],Y-m-d H:m:i)}&nbsp;&nbsp;点击:{$vo['hits']}</span>
