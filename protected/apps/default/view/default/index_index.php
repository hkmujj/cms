                ...
                {news:{table=(news) field=(id,title,picture,method,description) place=(101) where=(ispass='1') limit=(2)}}
                    <li {if $news_i==1}class="border-bottom padding-big-bottom"{/if}>
                        <div class="media media-x">
                          ...
                          <div><strong>[news:title]</strong>[news:description]... <a class="button button-little bg-blue swing-hover" href="[news:url]">详细</a></div>
                        </div>
                    </li>
                 {/news} 
                </ul>
          ...
          <div class="tab-body tab-body-bordered">
            <div class="tab-panel active" id="tab-start2">
               <ul class="list-unstyle height-big">
                    {news:{table=(news) field=(id,title,color,addtime,method,description) column=(100004) where=(ispass='1') limit=(7)}}
                       {if $news_i==1} <!--通过计数器判断第一条显示为头条样式-->
                         <li>
                            <a href="[news:url]"><h2 class="text-center">[news:title $len=20]</h2></a>
                            <p class="text-indent">[news:description]...<a href="[news:url]" class="text-blue">【详细】</a></p>
                         </li>
                       {else}
                          <li><a title="[news:title]" target="_blank" href="[news:url]">[news:title $len=25]</a><span class="float-right text-gray hidden-l text-little">{date($news['addtime'],Y-m-d)}</span></li>
                       {/if}
                    {/news} 
            ...
            <div class="tab-panel" id="tab-css2">
                <ul class="list-unstyle height-big">
                    {news:{table=(news) field=(id,title,color,addtime,method,description) column=(100005) where=(ispass='1') limit=(7)}}
                       {if $news_i==1} <!--通过计数器判断第一条显示为头条样式-->
                         <li>
                            <a href="[news:url]"><h2 class="text-center">[news:title $len=20]</h2></a>
                            <p class="text-indent">[news:description]...<a href="[news:url]" class="text-blue">【详细】</a></p>
                       ...
                       {else}
                          <li><!--a class="button button-little bg-blue" href="#">分类信息</a--> <a title="[news:title]" target="_blank" href="[news:url]">[news:title $len=25]</a><span class="float-right text-gray hidden-l text-little">{date($news['addtime'],Y-m-d)}</span></li>
                       {/if}
                    {/news} 
            ...
            <div class="tab-panel" id="tab-units2">
                <ul class="list-unstyle height-big">
                    {news:{table=(news) field=(id,title,color,addtime,method,description) column=(100006) where=(ispass='1') limit=(7)}}
                       {if $news_i==1} <!--通过计数器判断第一条显示为头条样式-->
                         <li>
                            <a href="[news:url]"><h2 class="text-center">[news:title $len=20]</h2></a>
                            <p class="text-indent">[news:description]...<a href="[news:url]" class="text-blue">【详细】</a></p>
...
<div class="container margin-top fadein-bottom">
         <div class="panel">
            ...
            <div class="panel-body">
                ...
                {photo:{table=(photo) field=(id,title,picture,method) exfield=(jiage) column=(100011) where=(ispass='1') limit=(4)}}
                  <div class="xl6 xs6 xm3 padding-bottom">
                    <div class="media media-y clearfix">
                        ...
                        <div class="media-body">[photo:title]&nbsp;<span class="tag bg-red">￥[photo:jiage]</span></div>
                    </div>
                  </div>
                {/photo}
                </div>
