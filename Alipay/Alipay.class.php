	       ...
	       //支付类型
            ...
            // $alipay->sendRequest();
             ...
             private function setConfig(){
                  ...
                  $this->notify_url = "http://{$_SERVER['HTTP_HOST']}/payr.php";
                  $this->return_url = "http://{$_SERVER['HTTP_HOST']}/payr.php?payr=1";

             }
            
             //建立请求,发送表单数据
             public function sendRequest($button_name = "确认"){
                      $html_text = $this->buildRequestForm($this->parameter,"post", $button_name);
                      return $html_text;
             }
             ...
             private function add_requestForm($para){
                ...
                $res = insert_data('xp_requestform',$data);
                if($res){
                  return true;
                }else{
                  return false;
            ...
            private  function buildRequestForm($para_temp, $method, $button_name) {
                ...
                $sHtml = $sHtml.'<input id="paybtn_'.$para['out_trade_no'].'" type="submit" value="'.$button_name.'" class="btn custom large" style="float: left;"></form>';
 ...
 */
  ...
  private function add_notifyform($post){
      ...
      if(insert_data('xp_notifyform',$data)){
        return true;
      }else{
        return false;
      }
